...
function generateUuid(): string {
  ...
  // Force v4 + variant bits per RFC 4122. `bytes` is a fixed-size
  // Uint8Array we just allocated, so indices 6 and 8 are always
  // present; the `?? 0` only satisfies noUncheckedIndexedAccess.
